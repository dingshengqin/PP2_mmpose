...
def visualize_model(model_cfg: Union[str, mmengine.Config],
    ...
    # 加载配置
    deploy_cfg, model_cfg = load_config(deploy_cfg, model_cfg)
    print("Loaded deploy config:", deploy_cfg)  # 打印部署配置
    print("Loaded model config:", model_cfg)  # 打印模型配置

    from mmdeploy.apis.utils import build_task_processor
    task_processor = build_task_processor(model_cfg, deploy_cfg)

    input_shape = get_input_shape(deploy_cfg)
    print("Input shape:", input_shape)  # 打印输入形状

    if backend is None:
        backend = get_backend(deploy_cfg)
    print("Using backend:", backend)  # 打印使用的后端
    ...
    if isinstance(model, (list, tuple)):
        ...
        if backend == Backend.PYTORCH:
            model = task_processor.build_pytorch_model(model[0])
            print("Built PyTorch model.")  # 打印模型构建信息
        else:
            model = task_processor.build_backend_model(
                model,
                data_preprocessor_updater=task_processor.update_data_preprocessor)
            print("Built backend model.")  # 打印后端模型构建信息

    model_inputs, _ = task_processor.create_input(img, input_shape)
    print("Model inputs created:", model_inputs)  # 打印模型输入

    with torch.no_grad():
        result = model.test_step(model_inputs)[0]
        print("Inference result:", result)  # 打印推理结果
    ...
    for single_img in img:
        print(f"Visualizing image: {single_img}")  # 打印正在可视化的图像
        task_processor.visualize(
            image=single_img,
            model=model,
            result=result,
            ...
            window_name=backend.value,
            show_result=show_result)
        print(f"Visualization completed for image: {single_img}")  # 打印可视化完成信息
